...
class VideoPlatformsParser
{
    ...
    public function get($url)
    {
        ...
        if (empty($info['tags']) && !empty($info['title'])) {
            ...
            $info['tags'] = array_map(function ($val) {
                return iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', $val);
            }, $info['tags']);
            $info['tags'] = array_map('strtolower', $info['tags']);
            $info['tags'] = preg_replace('/[^a-z0-9 ]+/', '', $info['tags']);
